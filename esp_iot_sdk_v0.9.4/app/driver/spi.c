...
void spi_master_init(uint8 spi_no)
{
   ...
   WRITE_PERI_REG(PERIPHS_IO_MUX, 0x105);//|(0x0001 << 9)); //clear bit9

   PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTDI_U, 2);//configure io to spi mode GPIO 12 - MISO
   PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTCK_U, 2);//configure io to spi mode GPIO 13 - MOSI
   PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTMS_U, 2);//configure io to spi mode GPIO 14 - CLK 
   PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTDO_U, 2);//configure io to spi mode GPIO 15 - CS


   //spi clock =  80 / 10*2 == 4M
   WRITE_PERI_REG(SPI_CLOCK(1),
               //((3 & SPI_CLKDIV_PRE) << SPI_CLKDIV_PRE_S) |
               //((0x07 & SPI_CLKDIV_PRE) << SPI_CLKDIV_PRE_S) |       //main clk pre divider
               ((0x07 & SPI_CLKDIV_PRE) << SPI_CLKDIV_PRE_S) |       //main clk pre divider
               ((0x04 & SPI_CLKCNT_N) << SPI_CLKCNT_N_S) |         //8 clock pulse
               ((0x04 & SPI_CLKCNT_H) << SPI_CLKCNT_H_S) |   //no of high clk
               ((0x02 & SPI_CLKCNT_L) << SPI_CLKCNT_L_S)); //
  
/*   WRITE_PERI_REG(SPI_CLOCK(1),
               //((3 & SPI_CLKDIV_PRE) << SPI_CLKDIV_PRE_S) |
               ((0x09 & SPI_CLKDIV_PRE) << SPI_CLKDIV_PRE_S) |       //main clk pre divider
               ((0x01 & SPI_CLKCNT_N) << SPI_CLKCNT_N_S) |         //8 clock pulse
               (((0x02 >> 1) & SPI_CLKCNT_H) << SPI_CLKCNT_H_S) |   //no of high clk
               ((0 & SPI_CLKCNT_L) << SPI_CLKCNT_L_S));*/
   SET_PERI_REG_MASK(SPI_USER(1), SPI_USR_MOSI|SPI_CS_SETUP|SPI_CS_HOLD);// | SPI_USR_MOSI | SPI_WR_BYTE_ORDER );
   //SET_PERI_REG_MASK(SPI_USER(1), SPI_USR_MOSI);
   CLEAR_PERI_REG_MASK(SPI_USER(1), SPI_FLASH_MODE | SPI_USR_MISO | SPI_WR_BYTE_ORDER | SPI_USR_ADDR | SPI_USR_DUMMY | SPI_USR_COMMAND);
   //CLEAR_PERI_REG_MASK(SPI_USER(1), SPI_FLASH_MODE | SPI_BYTE_ORDER);
/*   WRITE_PERI_REG(SPI_CLOCK(1),
               //((3 & SPI_CLKDIV_PRE) << SPI_CLKDIV_PRE_S) |
...
*/
 //   SET_PERI_REG_MASK(SPI_CTRL2(HSPI), (SPI_CK_OUT_HIGH_MODE<<SPI_CK_OUT_HIGH_MODE_S));
}
...
/*	uint32 regvalue;
	...
	CLEAR_PERI_REG_MASK(SPI_USER(spi_no), SPI_USR_MOSI|SPI_USR_MISO|SPI_USR_COMMAND|SPI_USR_ADDR|SPI_USR_DUMMY);
        ...
        WRITE_PERI_REG(SPI_USER1(spi_no), ((-1)&SPI_USR_ADDR_BITLEN)<<SPI_USR_ADDR_BITLEN_S | //Number of bits in Address
        ...
        if(READ_PERI_REG(SPI_USER(spi_no))&SPI_WR_BYTE_ORDER) {
          WRITE_PERI_REG(SPI_W0(spi_no), (uint32)data<<24);
        } else {
          WRITE_PERI_REG(SPI_W0(spi_no), 8);
        }
	...
	if(READ_PERI_REG(0x3ff00020)&BIT4){		
		...
		if(regvalue&SPI_SLV_WR_BUF_DONE){ 
            		...
            		while(idx<8){
            			...
            			idx++;
                                uart0_tx_buffer(spi_data[idx], 1); 
			}
			//add system_os_post here
            		GPIO_OUTPUT_SET(0, 1);
                        //uart0_tx_buffer(spi_data, 32); 
                        //uart0_tx_buffer("\r\n", 2); 
		}
